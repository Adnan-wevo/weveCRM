<flux:modal wire:model="show" class="w-full max-w-sm">
    <div class="mb-4 flex items-center gap-3">
        ...
        <div>
            <flux:heading>{{ __('Delete Selected Permissions') }}</flux:heading>
    ...
    <p class="rounded-lg bg-zinc-50 px-4 py-3 text-sm text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300">
        ...
        <strong class="font-semibold text-zinc-900 dark:text-white">{{ count($ids) }} {{ __('permissions') }}</strong>.
        {{ __('They will be detached from all roles.') }}
    </p>
