...
<div class="space-y-5">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <flux:heading size="xl" class="font-semibold">{{ __('Permissions') }}</flux:heading>
            <flux:text class="mt-0.5 text-zinc-500 dark:text-zinc-400">
                {{ __('Manage system permissions assigned to roles.') }}
            </flux:text>
        </div>
        <div class="flex flex-wrap items-center gap-2">
            @can('access-control.permissions.sync')
            ...
            @endif
            @endcan
            @can('access-control.permissions.import-export')
            ...
            @if ($tab === 'active')
            <flux:button icon="plus" wire:click="openCreate" variant="primary" class="shrink-0">{{ __('New Permission') }}</flux:button>
            @endif
            @endcan
        </div>
    </div>
    ...
    <div
        ...
        <div class="space-y-3 p-4">
            <flux:input wire:model.live.debounce.300ms="search" placeholder="{{ __('Search permission names…') }}"
    ...
    <div
        ...
        <div class="flex items-center justify-between border-b border-zinc-100 px-4 py-3 dark:border-zinc-800">
            <div class="flex items-center gap-2">
                <span class="text-sm text-zinc-500 dark:text-zinc-400">
                    {{ $this->permissions->total() }} {{ __('permissions') }}
                </span>
                @if (count($selected) > 0)
            ...
            <div class="flex items-center gap-2">
                @can('access-control.permissions.destroy')
                @if (count($selected) > 0)
            ...
            <flux:table>
                ...
                <flux:table.rows>
                    @forelse ($this->permissions as $permission)
                        <flux:table.row :key="$permission->id" wire:key="perm-{{ $permission->id }}">
                            <flux:table.cell class="px-4">
                                <flux:checkbox wire:model.live="selected" value="{{ $permission->id }}" />
                            </flux:table.cell>

                            <flux:table.cell class="text-center text-xs tabular-nums text-zinc-400 dark:text-zinc-500">
                                {{ $this->permissions->firstItem() + $loop->index }}
                            </flux:table.cell>

                            <flux:table.cell>
                                <div class="flex items-center gap-2">
                                    <div
                                        class="flex size-7 items-center justify-center rounded-full bg-emerald-100 dark:bg-emerald-900/40">
                                        <flux:icon name="key"
                                            class="size-3.5 text-emerald-600 dark:text-emerald-300" />
                                    </div>
                                    <span
                                        class="font-medium text-zinc-800 dark:text-zinc-100">{{ $permission->name }}</span>
                            ...
                            <flux:table.cell class="text-end">
                                <flux:dropdown>
                                    ...
                                    <flux:menu>
                                        @if ($tab === 'active')
                                        @can('access-control.permissions.show')
                                        <flux:menu.item icon="eye" wire:click="openShow('{{ $permission->id }}')">
                                            {{ __('View') }}
                                        </flux:menu.item>
                                        @endcan
                                        @can('access-control.permissions.history')
                                        <flux:menu.separator />
                                        <flux:menu.item icon="clock" wire:click="openHistory('{{ $permission->id }}')">
                                            {{ __('History') }}
                                        </flux:menu.item>
                                        @endcan
                                        @can('access-control.permissions.edit')
                                        <flux:menu.separator />
                                        @if (isset($this->lockedIds[(string) $permission->id]))
                                            <flux:menu.item icon="lock-closed" disabled>
                                                {{ __('Editing — :name', ['name' => $this->lockedIds[(string) $permission->id]['user_name']]) }}
                                            </flux:menu.item>
                                        @else
                                            <flux:menu.item icon="pencil-square"
                                                wire:click="openEdit('{{ $permission->id }}')">{{ __('Edit') }}</flux:menu.item>
                                        @endif
                                        @endcan
                                        @can('access-control.permissions.destroy')
                                        <flux:menu.separator />
                                        ...
                                        @else
                                            <flux:menu.item icon="trash"
                                                wire:click="openDelete('{{ $permission->id }}')">{{ __('Delete') }}</flux:menu.item>
                                        @endif
                                        @endcan
                                        @else
                                        @can('access-control.permissions.restore')
                                        <flux:menu.item icon="arrow-path" wire:click="openRestore('{{ $permission->id }}')">
                                            {{ __('Restore') }}
                                        </flux:menu.item>
                                        @endcan
                                        @can('access-control.permissions.force-delete')
                                        <flux:menu.separator />
                                        <flux:menu.item icon="fire" wire:click="openForceDelete('{{ $permission->id }}')">
                                            {{ __('Delete Forever') }}
                                        </flux:menu.item>
                                        @endcan
                                        @endif
                        ...
                        <flux:table.row>
                            <flux:table.cell colspan="7" class="py-16 text-center">
                                <div class="flex flex-col items-center gap-2">
                                    <flux:icon name="key" class="size-10 text-zinc-300 dark:text-zinc-600" />
                                    <p class="text-sm font-medium text-zinc-500 dark:text-zinc-400">
                                        {{ __('No permissions found') }}</p>
        ...
        <div class="flex flex-col items-center gap-2 border-t border-zinc-100 px-4 py-3 dark:border-zinc-800 sm:flex-row sm:items-center sm:justify-between">
            <span class="text-xs text-zinc-500 dark:text-zinc-400">
                {{ __('Showing') }}
                <strong>{{ $this->permissions->firstItem() ?? 0 }}</strong>–<strong>{{ $this->permissions->lastItem() ?? 0 }}</strong>
                {{ __('of') }} <strong>{{ $this->permissions->total() }}</strong> {{ __('permissions') }}
            </span>
    ...
    {{-- Modal Components --}}
    @livewire('accesscontrol::permissions.show-modal')
    @livewire('accesscontrol::permissions.create-modal')
    @livewire('accesscontrol::permissions.edit-modal')
    @livewire('accesscontrol::permissions.delete-modal')
    @livewire('accesscontrol::permissions.bulk-delete-modal')
    @livewire('accesscontrol::permissions.import-export-modal')
    @livewire('accesscontrol::permissions.restore-modal')
    @livewire('accesscontrol::permissions.force-delete-modal')
    @livewire('accesscontrol::permissions.history-modal')
    @livewire('accesscontrol::permissions.sync-modal')
