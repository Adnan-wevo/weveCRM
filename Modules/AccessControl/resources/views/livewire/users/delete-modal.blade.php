<flux:modal wire:model="show" class="w-full max-w-sm">
    ...
    <p class="rounded-lg bg-zinc-50 px-4 py-3 text-sm text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300">
        {{ __('You are about to delete') }} <strong class="font-semibold text-zinc-900 dark:text-white">{{ $userName }}</strong>.
        {{ __('All data associated with this user will be permanently removed.') }}
    </p>

    <div class="mt-5 flex justify-end gap-2">
        <flux:button variant="filled" wire:click="$set('show', false)">{{ __('Cancel') }}</flux:button>
        <flux:button variant="danger" wire:click="delete" wire:loading.attr="disabled">{{ __('Delete User') }}</flux:button>
    </div>
</flux:modal>
