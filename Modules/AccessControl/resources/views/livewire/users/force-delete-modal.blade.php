<flux:modal wire:model="show" class="w-full max-w-sm">
    <div class="mb-4 flex items-center gap-3">
        ...
        <div>
            <flux:heading>{{ __('Permanently Delete User') }}</flux:heading>
    ...
    <p class="rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700 dark:bg-red-900/20 dark:text-red-300">
        {{ __('You are about to permanently erase') }} <strong class="font-semibold">{{ $userName }}</strong>.
        {{ __('All data, media, and audit logs linked to this user will be irreversibly removed from the database.') }}
    </p>
