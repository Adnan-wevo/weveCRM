<flux:modal wire:model="show" class="w-full max-w-2xl">
    ...
    <div class="mb-5">
        <flux:heading size="lg">{{ __('Import / Export Users') }}</flux:heading>
        <flux:text class="text-zinc-500">
            {{ __('Export existing users to a spreadsheet or import new users from a file.') }}
        </flux:text>
    </div>
        ...
        <div class="space-y-6">
            ...
            <div class="rounded-lg border border-zinc-200 p-4 dark:border-zinc-700">
                ...
                <flux:text class="mb-4 text-sm text-zinc-500">
                    {{ __('Download all users as an Excel spreadsheet including their roles.') }}
                </flux:text>
            ...
            <div class="rounded-lg border border-zinc-200 p-4 dark:border-zinc-700">
                ...
                <flux:text class="mb-1 text-sm text-zinc-500">
                    {{ __('Upload a CSV or Excel file to create new users. Expected columns:') }}
                </flux:text>
                <p class="mb-4 font-mono text-xs text-zinc-400">name, email, password, roles</p>
            ...
            <table class="min-w-full divide-y divide-zinc-200 text-sm dark:divide-zinc-700">
                <thead class="bg-zinc-50 dark:bg-zinc-800">
                    <tr>
                        ...
                        <th class="px-3 py-2 text-left text-xs font-medium uppercase text-zinc-500">{{ __('Email') }}</th>
                        <th class="px-3 py-2 text-left text-xs font-medium uppercase text-zinc-500">{{ __('Roles') }}</th>
                ...
                <tbody class="divide-y divide-zinc-100 bg-white dark:divide-zinc-800 dark:bg-zinc-900">
                    @foreach ($previewRows as $i => $row)
                        <tr class="{{ $row['valid'] ? '' : 'bg-red-50 dark:bg-red-900/10' }}">
                            ...
                            <td class="px-3 py-2">{{ $row['data']['email'] ?? '—' }}</td>
                            <td class="px-3 py-2 text-zinc-500">{{ $row['data']['roles'] ?? '—' }}</td>
                            <td class="px-3 py-2">
                                @if ($row['valid'])
