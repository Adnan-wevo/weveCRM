<flux:modal wire:model="show" class="w-full max-w-sm">
    <div class="mb-4 flex items-center gap-3">
        ...
        <div>
            <flux:heading>{{ __('Restore User') }}</flux:heading>
            <flux:text class="text-zinc-500">{{ __('This user will be made active again.') }}</flux:text>
        </div>
    </div>

    <p class="rounded-lg bg-zinc-50 px-4 py-3 text-sm text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300">
        {{ __('You are about to restore') }} <strong class="font-semibold text-zinc-900 dark:text-white">{{ $userName }}</strong>.
        {{ __('They will regain access based on their assigned roles.') }}
    </p>

    <div class="mt-5 flex justify-end gap-2">
        <flux:button variant="filled" wire:click="$set('show', false)">{{ __('Cancel') }}</flux:button>
        <flux:button variant="primary" wire:click="restore" wire:loading.attr="disabled">{{ __('Restore User') }}</flux:button>
    </div>
</flux:modal>
