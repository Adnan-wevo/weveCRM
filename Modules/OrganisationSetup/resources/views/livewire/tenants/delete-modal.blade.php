<flux:modal wire:model="show" class="w-full max-w-sm">
    <div class="mb-4 flex items-center gap-3">
        ...
        <div>
            <flux:heading>{{ __('Delete Tenant') }}</flux:heading>
    ...
    <p class="rounded-lg bg-zinc-50 px-4 py-3 text-sm text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300">
        {{ __('You are about to delete') }} <strong class="font-semibold text-zinc-900 dark:text-white">{{ $tenantName }}</strong>.
        {{ __('The tenant will be moved to the trash and can be restored later.') }}
    </p>
