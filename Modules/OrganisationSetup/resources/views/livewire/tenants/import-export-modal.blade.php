<flux:modal wire:model="show" class="w-full max-w-2xl">
    ...
    <div class="mb-5">
        <flux:heading size="lg">{{ __('Import / Export Tenants') }}</flux:heading>
        <flux:text class="text-zinc-500">
            {{ __('Export existing tenants to a spreadsheet or import new tenants from a file.') }}
        </flux:text>
    </div>
        ...
        <div class="space-y-6">
            ...
            <div class="rounded-lg border border-zinc-200 p-4 dark:border-zinc-700">
                ...
                <flux:text class="mb-4 text-sm text-zinc-500">
                    {{ __('Download all tenants as an Excel spreadsheet.') }}
                </flux:text>
                ...
                @else
                    <flux:button icon="arrow-down-tray" wire:click="export" wire:loading.attr="disabled" variant="outline" class="shrink-0 !text-amber-600 !border-amber-400 hover:!bg-amber-50 dark:!text-amber-400 dark:!border-amber-600 dark:hover:!bg-amber-950">
                        {{ __('Export Tenants') }}
                    </flux:button>
                @endif
            </div>
            ...
            <div class="rounded-lg border border-zinc-200 p-4 dark:border-zinc-700">
                ...
                <flux:text class="mb-1 text-sm text-zinc-500">
                    {{ __('Upload a CSV or Excel file to create new tenants. Expected columns:') }}
                </flux:text>
                <p class="mb-4 font-mono text-xs text-zinc-400">id, name</p>
    ...
    @elseif ($step === 'preview')
            ...
            <table class="min-w-full divide-y divide-zinc-200 text-sm dark:divide-zinc-700">
                <thead class="bg-zinc-50 dark:bg-zinc-800">
                    <tr>
                        ...
                        <th class="px-3 py-2 text-left text-xs font-medium uppercase text-zinc-500">{{ __('ID') }}</th>
                ...
                <tbody class="divide-y divide-zinc-100 bg-white dark:divide-zinc-800 dark:bg-zinc-900">
                    @foreach ($previewRows as $i => $row)
                        <tr class="{{ $row['valid'] ? '' : 'bg-red-50 dark:bg-red-900/10' }}">
                            ...
                            <td class="px-3 py-2 font-mono text-xs text-zinc-500">{{ $row['data']['id'] ?? '—' }}</td>
        ...
        <div class="mt-6 flex justify-end gap-2">
            <flux:button variant="filled" wire:click="backToIdle">{{ __('Back') }}</flux:button>
            @if ($validCount > 0)
