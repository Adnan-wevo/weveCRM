...
<div class="space-y-5">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <flux:heading size="xl" class="font-semibold">{{ __('Tenants') }}</flux:heading>
            <flux:text class="mt-0.5 text-zinc-500 dark:text-zinc-400">
                {{ __('Manage tenants in your organisation.') }}
            </flux:text>
        </div>
        <div class="flex flex-wrap items-center gap-2">
            @can('organisationsetup::tenants.sync-migrations')
            <flux:button icon="arrow-path" wire:click="openSyncMigrations" variant="outline" class="shrink-0 !text-blue-600 !border-blue-400 hover:!bg-blue-50 dark:!text-blue-400 dark:!border-blue-600 dark:hover:!bg-blue-950">{{ __('Sync Migrations & Seed') }}</flux:button>
            @endcan
            @can('organisationsetup::tenants.import-export')
            ...
            @endcan
            @can('organisationsetup::tenants.create')
            @if ($tab === 'active')
            <flux:button icon="plus" wire:click="openCreate" variant="primary" class="shrink-0">{{ __('New Tenant') }}</flux:button>
            @endif
            @endcan
        </div>
    </div>
    ...
    <div
        ...
        <div
            ...
            <div class="flex items-center justify-between gap-2 sm:justify-end">
                @if ($search || count($filterRows) > 0 || $sortBy !== 'id' || $sortDir !== 'asc')
        ...
        <div class="space-y-3 p-4">
            <flux:input wire:model.live.debounce.300ms="search" placeholder="{{ __('Search tenant names…') }}"
    ...
    <div
        ...
        <div class="flex items-center justify-between border-b border-zinc-100 px-4 py-3 dark:border-zinc-800">
            ...
            <div class="flex items-center gap-2">
                @can('organisationsetup::tenants.destroy')
                @if (count($selected) > 0 && $tab === 'active')
        ...
        <div class="m-4 px-4 py-3">
            <flux:table>
                <flux:table.columns>
                    ...
                    <flux:table.column sortable :sorted="$sortBy === 'id'" :direction="$sortDir"
                        wire:click="sort('id')" class="cursor-pointer">
                        {{ __('ID') }}
                    </flux:table.column>
                ...
                <flux:table.rows>
                    @forelse ($this->tenants as $tenant)
                        <flux:table.row :key="$tenant->id" wire:key="tenant-{{ $tenant->id }}">
                            <flux:table.cell class="px-4">
                                <flux:checkbox wire:model.live="selected" value="{{ $tenant->id }}" />
                            </flux:table.cell>

                            <flux:table.cell class="text-center text-xs tabular-nums text-zinc-400 dark:text-zinc-500">
                                {{ $this->tenants->firstItem() + $loop->index }}
                            </flux:table.cell>

                            <flux:table.cell>
                                <flux:badge size="sm" color="zinc">{{ $tenant->id }}</flux:badge>
                            </flux:table.cell>

                            <flux:table.cell>
                                <div class="flex items-center gap-2">
                                    <div
                                        class="flex size-7 items-center justify-center rounded-full bg-blue-100 dark:bg-blue-900/40">
                                        <flux:icon name="building-office"
                                            class="size-3.5 text-blue-600 dark:text-blue-300" />
                                    </div>
                                    <span
                                        class="font-medium text-zinc-800 dark:text-zinc-100">{{ $tenant->name }}</span>
                                </div>
                            </flux:table.cell>

                            <flux:table.cell>
                                <flux:badge size="sm" color="indigo">
                                    {{ $tenant->domains_count }} {{ __('domains') }}
                                </flux:badge>
                            </flux:table.cell>
                            ...
                            <flux:table.cell class="text-end">
                                <flux:dropdown>
                                    ...
                                    <flux:menu>
                                        @if ($tab === 'active')
                                        @can('organisationsetup::tenants.show')
                                        <flux:menu.item icon="eye" wire:click="openShow('{{ $tenant->id }}')">
                                            {{ __('View') }}
                                        </flux:menu.item>
                                        @endcan
                                        @can('organisationsetup::tenants.history')
                                        <flux:menu.separator />
                                        <flux:menu.item icon="clock" wire:click="openHistory('{{ $tenant->id }}')">
                                            {{ __('History') }}
                                        </flux:menu.item>
                                        @endcan
                                        @can('organisationsetup::tenants.add-user')
                                        <flux:menu.separator />
                                        <flux:menu.item icon="users" wire:click="openTenantUsers('{{ $tenant->id }}')">
                                            {{ __('Manage Users') }}
                                        </flux:menu.item>
                                        @endcan
                                        @can('organisationsetup::tenants.assign-domain')
                                        <flux:menu.separator />
                                        <flux:menu.item icon="globe-alt" wire:click="openAssignDomain('{{ $tenant->id }}')">
                                            {{ __('Assign Domains') }}
                                        </flux:menu.item>
                                        @endcan
                                        @can('organisationsetup::tenants.edit')
                                        <flux:menu.separator />
                                        @if (isset($this->lockedIds[(string) $tenant->id]))
                                            <flux:menu.item icon="lock-closed" disabled>
                                                {{ __('Editing — :name', ['name' => $this->lockedIds[(string) $tenant->id]['user_name']]) }}
                                            </flux:menu.item>
                                        @else
                                            <flux:menu.item icon="pencil-square"
                                                wire:click="openEdit('{{ $tenant->id }}')">{{ __('Edit') }}</flux:menu.item>
                                        @endif
                                        @endcan
                                        @can('organisationsetup::tenants.destroy')
                                        <flux:menu.separator />
                                        ...
                                        @else
                                            <flux:menu.item icon="trash"
                                                wire:click="openDelete('{{ $tenant->id }}')">{{ __('Delete') }}</flux:menu.item>
                                        @endif
                                        @endcan
                                        @else
                                        @can('organisationsetup::tenants.restore')
                                        <flux:menu.item icon="arrow-path" wire:click="openRestore('{{ $tenant->id }}')">
                                            {{ __('Restore') }}
                                        </flux:menu.item>
                                        @endcan
                                        @can('organisationsetup::tenants.force-delete')
                                        <flux:menu.separator />
                                        <flux:menu.item icon="fire" wire:click="openForceDelete('{{ $tenant->id }}')">
                                            {{ __('Delete Forever') }}
                                        </flux:menu.item>
                                        @endcan
                                        @endif
                        ...
                        <flux:table.row>
                            <flux:table.cell colspan="7" class="py-16 text-center">
                                <div class="flex flex-col items-center gap-2">
                                    <flux:icon name="building-office" class="size-10 text-zinc-300 dark:text-zinc-600" />
                                    <p class="text-sm font-medium text-zinc-500 dark:text-zinc-400">
                                        {{ __('No tenants found') }}</p>
        ...
        <div class="flex flex-col items-center gap-2 border-t border-zinc-100 px-4 py-3 dark:border-zinc-800 sm:flex-row sm:items-center sm:justify-between">
            <span class="text-xs text-zinc-500 dark:text-zinc-400">
                {{ __('Showing') }}
                <strong>{{ $this->tenants->firstItem() ?? 0 }}</strong>–<strong>{{ $this->tenants->lastItem() ?? 0 }}</strong>
                {{ __('of') }} <strong>{{ $this->tenants->total() }}</strong> {{ __('tenants') }}
            </span>
            {{ $this->tenants->links() }}
        </div>
    ...
    {{-- Modal Components --}}
    @livewire('organisationsetup::tenants.show-modal')
    @livewire('organisationsetup::tenants.create-modal')
    @livewire('organisationsetup::tenants.edit-modal')
    @livewire('organisationsetup::tenants.delete-modal')
    @livewire('organisationsetup::tenants.bulk-delete-modal')
    @livewire('organisationsetup::tenants.import-export-modal')
    @livewire('organisationsetup::tenants.restore-modal')
    @livewire('organisationsetup::tenants.force-delete-modal')
    @livewire('organisationsetup::tenants.tenant-user-modal')
    @livewire('organisationsetup::tenants.assign-domain-modal')
    @livewire('organisationsetup::tenants.history-modal')
    @livewire('organisationsetup::tenants.sync-migrations-modal')
